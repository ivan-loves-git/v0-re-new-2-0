...
/** A server-streamed private download after exact dossier/attachment authorization. */
export async function GET(
  ...
  if (!access || access.role === "unassigned") return privateStorageDownloadError("Not found", 404)
  ...
  if (error || !data?.[0]) return privateStorageDownloadError("Not found", 404)
  ...
  if (signedError || !signed?.signedUrl) {
    return privateStorageDownloadError("Unable to open attachment.", 500)
  }
