...
export default async function StrategicPdrRequestDetail({ params }: { params: Promise<{ requestId: string }> }) {
  ...
  const dispositionEligible = isDispositionEligiblePdrRequest({ provenance: request.provenance, status: request.screening.status, requesterActor: request.requester.actor, requesterUserId: request.requester.userId, dispositionKind: request.disposition.kind })
