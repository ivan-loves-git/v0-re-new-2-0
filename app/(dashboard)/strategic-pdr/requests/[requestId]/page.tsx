...
export default async function StrategicPdrRequestDetail({ params }: { params: Promise<{ requestId: string }> }) {
  ...
  return <div className="space-y-6"><div><p className="wave-micro-label">Strategic PDR request</p><h1 className="text-2xl font-semibold">{request.title}</h1><p className="text-sm text-muted-foreground">Submitted by {request.requester.displayName ?? request.requester.legacyLabel}</p></div>
    ...
    <Card><CardHeader><CardTitle>Screening record</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><p>{request.screening.problemStatement}</p><p className="text-muted-foreground">{request.screening.aiRationale || "Screening has not been completed."}</p></CardContent></Card>
    ...
    {canDisposition && !request.disposition.kind ? <Card><CardHeader><CardTitle>Ivan disposition</CardTitle></CardHeader><CardContent><form action={dispositionStrategicPdrRequest} className="space-y-3"><input type="hidden" name="request_id" value={request.id}/><Textarea name="reviewer_note" maxLength={2000} placeholder="Reason or boundary (optional)"/><div className="flex gap-2"><Button name="disposition" value="approved">Approve intake</Button><Button name="disposition" value="declined" variant="outline">Decline intake</Button></div></form></CardContent></Card> : <Card><CardHeader><CardTitle>Disposition</CardTitle></CardHeader><CardContent className="text-sm">{request.disposition.kind ?? "Awaiting Ivan's disposition."}</CardContent></Card>}
  </div>
}
