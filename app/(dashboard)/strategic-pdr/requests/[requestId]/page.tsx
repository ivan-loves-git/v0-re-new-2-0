...
export default async function StrategicPdrRequestDetail({ params }: { params: Promise<{ requestId: string }> }) {
  ...
  return <div className="space-y-6"><div><p className="wave-micro-label">Strategic PDR request</p><h1 className="text-2xl font-semibold">{request.title}</h1><p className="text-sm text-muted-foreground">Submitted by {request.requester.displayName ?? request.requester.legacyLabel}</p></div>
    ...
    {request.githubProductChange ? <Card><CardHeader><CardTitle>Product delivery</CardTitle></CardHeader><CardContent className="text-sm"><a className="underline" href={request.githubProductChange.url} target="_blank" rel="noreferrer">Open / discuss Product Change #{request.githubProductChange.number} in GitHub</a></CardContent></Card> : null}
    ...
    {isCurrentStaffRequest ? <>
      ...
      {dispositionEligible ? <PdrScreeningEditor requestId={request.id}/> : request.disposition.kind ? <p className="text-muted-foreground">AI screening is closed after Ivan’s disposition.</p> : <p className="text-muted-foreground">AI screening is available only for current draft WAVE staff intake.</p>}
