...
export default async function DashboardPage() {
  ...
  try {
    ...
    const { data: repreneurs, error: repreneursError } = await supabase.from("repreneurs").select("*")

    if (repreneursError) {
      console.error("[v0] Dashboard: Error fetching repreneurs", repreneursError)
    }

    console.log("[v0] Dashboard: Repreneurs fetched", repreneurs?.length || 0)
    ...
    if (notesError) {
      console.error("[v0] Dashboard: Error fetching notes", notesError)
    }

    console.log("[v0] Dashboard: Recent notes fetched", recentNotes?.length || 0)
    ...
    console.log("[v0] Dashboard: Rendering complete")
    ...
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-semibold text-gray-900">Dashboard</h1>
          <p className="text-red-600 mt-1">Error loading dashboard. Check console for details.</p>
        </div>
        <Card>
