...
test.describe.serial("Golden journeys", () => {
  ...
  test("P3 proposal interest retry and staff validation agree after reload", async ({ browser }) => {
    ...
    // Build a real Gate-1 state, then classify its parent as DEMO while the
    // portal page remains open. This proves that stale browser state cannot
    // bypass the server-side veto on template, document, or signed-copy paths.
    const template = await database.query(`SELECT * FROM public.register_opportunity_nda_artifact(
      $1, NULL, 'blank_template', $2, $3, 'qa-template.pdf', 1, repeat('a', 64), $4
    )`, [
      opportunityId,
      `${manifest.fixturePrefix} NDA template`,
      `${opportunityId}/nda-artifacts/blank_template/qa-template.pdf`,
      manifest.actors.staff.email,
    ])
    const templateArtifactId = template.rows[0]?.artifact_id
    if (typeof templateArtifactId !== "string") throw new Error("Golden P3 failed: template-artifact-id")
    await database.query("SELECT public.journey_record_evidence($1, 'qualification_requested', $2, $3)", [matchId, manifest.actors.staff.email, `${manifest.runId}:qualification-requested`])
    ...
    const memo = await database.query(`INSERT INTO public.opportunity_documents (
      ...
      RETURNING id`, [
      opportunityId,
      `${manifest.fixturePrefix} information memo`,
      `${opportunityId}/documents/qa-information-memo.pdf`,
      manifest.actors.staff.email,
    ])
    const memoDocumentId = memo.rows[0]?.id
    if (typeof memoDocumentId !== "string") throw new Error("Golden P3 failed: memo-document-id")

    await portalOne.goto(`/portal/deals/${matchId}`)
    await expect(portalOne.getByRole("link", { name: "Download template" })).toBeVisible()
    await expect(portalOne.getByRole("button", { name: "Upload signed copy" })).toBeVisible()
    await database.query("UPDATE public.opportunities SET is_demo=true WHERE id=$1", [opportunityId])

    const directStatuses = await portalOne.evaluate(async ({ activeMatchId, documentId }) => {
      const [templateResponse, memoResponse] = await Promise.all([
        fetch(`/portal/deals/${activeMatchId}/nda-template`),
        fetch(`/portal/deals/${activeMatchId}/documents/${documentId}`),
      ])
      return { template: templateResponse.status, memo: memoResponse.status }
    }, { activeMatchId: matchId, documentId: memoDocumentId })
    expect(directStatuses).toEqual({ template: 404, memo: 404 })

    await portalOne.getByLabel("Signed PDF").setInputFiles({
      name: "qa-signed-copy.pdf",
      mimeType: "application/pdf",
      buffer: Buffer.from("synthetic signed NDA"),
    })
    await portalOne.getByRole("button", { name: "Upload signed copy" }).click()
    await expect(portalOne.getByRole("alert")).toContainText("This NDA is not available for upload.")
    const deniedUpload = await database.query(`SELECT
      (SELECT count(*)::int FROM public.opportunity_nda_artifacts WHERE match_id=$1 AND artifact_role='repreneur_signed_copy') AS artifacts,
      (SELECT count(*)::int FROM public.opportunity_documents WHERE opportunity_id=$2 AND storage_path LIKE $3) AS documents`, [
      matchId,
      opportunityId,
      `${opportunityId}/nda-artifacts/repreneur_signed_copy/%`,
    ])
    expect(deniedUpload.rows[0]).toMatchObject({ artifacts: 0, documents: 0 })
    await database.query("UPDATE public.opportunities SET is_demo=false WHERE id=$1", [opportunityId])
    ...
    const mobileInProgress = sectionFor(mobile, "In Progress")
    await expect(mobileInProgress.getByText(`${manifest.fixturePrefix} opportunity`, { exact: true })).toBeVisible()
    await expect(mobileInProgress.getByText("Active pursuit", { exact: true })).toBeVisible()
    await expect(mobileInProgress.getByText("Revenue", { exact: true })).toHaveCount(0)
