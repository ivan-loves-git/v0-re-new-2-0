...
export function StepNeeds({
  ...
  const removeFile = () => {
    void abandonPrivateIntakeUpload(thesisUpload.url)
    setThesisUpload({
      file: null,
      uploading: false,
      progress: 0,
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">{t("step5Description")}</p>
      </div>

      {/* Q17: Current Needs */}
      <div className="flex flex-col gap-3">
        <Label className="text-base font-medium">{t("q17Label")} *</Label>
      ...
      <div className="flex flex-col gap-3 p-4 border rounded-md bg-muted/30">
        <div
          ...
          onClick={() =>
            onChange({ marketing_consent: !data.marketing_consent })
          }
        >
          <Checkbox
            checked={data.marketing_consent || false}
            onCheckedChange={(checked) =>
              onChange({ marketing_consent: checked as boolean })
            }
      ...
      <div className="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-950/30 rounded-md text-sm">
        ...
        <p className="text-amber-700 dark:text-amber-300">{t("gdprNotice")}</p>
      </div>
