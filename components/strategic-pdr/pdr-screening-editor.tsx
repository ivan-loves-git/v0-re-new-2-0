...
export function PdrScreeningEditor({ requestId }: { requestId: string }) {
  const [preview, setPreview] = useState<{ draft: PdrScreeningDraft; context: PdrScreeningContext } | null>(null)
  ...
  const save = () => startTransition(async () => {
    if (!preview) return
    setError(null)
    try { await saveStrategicPdrScreening({ requestId, ...preview }); setPreview(null) } catch (cause) { setError(cause instanceof Error ? cause.message : "Screening could not be saved.") }
  })
