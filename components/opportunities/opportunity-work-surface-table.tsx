...
function sourceFirmName(opportunity: OpportunityWorkSurfaceRecord) {
  return (
...
function sourceContextLabel(opportunity: OpportunityWorkSurfaceRecord) {
  const firmName = sourceFirmName(opportunity)
...
function prepareOpportunity(
  ...
  return {
    ...
    activeMatch,
    interestedCount: opportunity.matches.filter(
      (match) => match.status === "interested",
    ).length,
...
function pursuitSummary(item: PreparedOpportunity) {
  ...
  if (item.opportunity.matches.length > 0)
    return `${item.opportunity.matches.length} match${item.opportunity.matches.length === 1 ? "" : "es"}`
  return "No pursuit"
}
...
export function OpportunityWorkSurfaceTable({
  opportunities,
  mode,
  ...
  const sourceFirms = useMemo(
    () =>
      Array.from(
        new Set(
          opportunities
            .map(sourceFirmName)
            .filter((firmName): firmName is string => Boolean(firmName)),
        ),
      ).sort(),
    [opportunities],
  )
    ...
    () => [
      {
        key: "journey",
        label: "Journey",
        options: OPPORTUNITY_JOURNEY_OPTIONS,
      },
      ...
      },
      {
        key: "sourceFirm",
        label: "Source firm",
        options: sourceFirms.map((firmName) => ({
          value: firmName,
          label: firmName,
        })),
      },
  ...
  const journeyFilter = (filters.values.journey || "all") as
    | OpportunityJourney
    | "all"
  const statusFilter = (filters.values.status || "all") as
    | OpportunityStatus
    | "all"
  ...
  const filtered = useMemo(() => {
    ...
    return prepared.filter((item) => {
      const { opportunity } = item
      if (journeyFilter !== "all" && item.journey !== journeyFilter)
        return false
      ...
      if (locationFilter !== "all" && opportunity.location !== locationFilter)
        return false
      if (
        sourceFirmFilter !== "all" &&
        sourceFirmName(opportunity) !== sourceFirmFilter
      ) {
        return false
      }
  ...
  const filterBar = (
    <CollectionFilterBar
      ...
      className={
        mode === "find" ? "rounded-none border-x-0 border-t-0" : undefined
      }
    />
  )
        ...
        <div className="flex flex-col gap-4">
          {GROUP_CONFIG.map((group) => {
            ...
            const groupTotalPages = Math.ceil(
              items.length / GROUP_ITEMS_PER_PAGE,
            )
                  ...
                  <div className="overflow-x-auto bg-card">
                    ...
                    {groupTotalPages > 1 && (
                      <div className="flex flex-col gap-2 border-t px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                        <span className="text-xs text-muted-foreground">
                          Showing {start + 1}-
                          {Math.min(start + GROUP_ITEMS_PER_PAGE, items.length)}{" "}
                          of {items.length}
                        </span>
                          ...
                          <PaginationContent>
                            <PaginationItem>
                              <PaginationPrevious
                                disabled={page === 1}
                                onClick={() =>
                                  page > 1 && setGroupPage(group.key, page - 1)
                                }
                                className={
                                  page === 1
                            ...
                            {Array.from(
                              ...
                              .filter(
                                (candidate) =>
                                  candidate === 1 ||
                                  candidate === groupTotalPages ||
                                  Math.abs(candidate - page) <= 1,
                              )
                              .map((candidate, index, shownPages) => {
                                const showEllipsis =
                                  index > 0 &&
                                  candidate - shownPages[index - 1] > 1
                                return (
                                  <span
                                    key={candidate}
                                  ...
                                  >
                                    ...
                                    <PaginationItem>
                                      <PaginationLink
                                        onClick={() =>
                                          setGroupPage(group.key, candidate)
                                        }
                            ...
                            <PaginationItem>
                              <PaginationNext
                                ...
                                onClick={() =>
                                  page < groupTotalPages &&
                                  setGroupPage(group.key, page + 1)
                                }
                                className={
                                  page === groupTotalPages
                                    ? "pointer-events-none opacity-50"
    ...
    <div className="overflow-hidden rounded-lg border bg-card">
            ...
            <TableBody>
              {paginated.length === 0 ? (
                <TableRow>
                  ...
                  >
                    No opportunities match these filters. Remove a filter to
                    widen the list.
                  </TableCell>
                </TableRow>
              ) : (
      ...
      {totalPages > 1 && (
        <div className="flex flex-col gap-2 border-t bg-muted/20 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between">
          <span className="text-xs text-muted-foreground">
            Showing {pageStart + 1}-
            {Math.min(pageStart + FIND_ITEMS_PER_PAGE, filtered.length)} of{" "}
            {filtered.length}
          </span>
          <Pagination className="sm:mx-0 sm:w-auto">
            <PaginationContent>
              ...
              {Array.from({ length: totalPages }, (_, index) => index + 1)
                ...
                .map((page, index, shownPages) => {
                  const showEllipsis =
                    index > 0 && page - shownPages[index - 1] > 1
                  return (
