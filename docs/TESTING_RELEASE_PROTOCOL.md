...
Classify the highest consequence introduced by the change. `Verify` is the universal automated check for runtime code. The permanent protected P1–P3 lane is selective, not universal: use it only for Tier 3. Tier 2 uses the relevant exact-candidate journey, while Tier 0 and Tier 1 do not dispatch Golden Journeys.
