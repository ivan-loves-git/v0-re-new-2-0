...
| `w128_draft_activation_runs` | Immutable staff-only audit evidence for a separately approved W-128 one-time Draft-to-Active run; it stores the ordered manifest, its SHA-256 digest, actor, time, activated count and rollback manifest | The preflight derives eligibility solely from the valid-opportunity contract: non-DEMO Draft, active source office beneath a non-archived firm, description, active same-office contacts, exactly one primary contact and a usable primary email. The apply primitive locks and revalidates every supplied row, rejects any identity, timestamp, fingerprint or eligibility drift, changes only `status` and `updated_by`, and preserves exposure, matches, pursuits, documents, references and source/contact links. It has no browser UI and never embeds a production candidate set. | W-128 integrated release candidate; a fresh production read-only preflight and Ivan's Gate 2 approval are required before any apply |
...
| --- | --- | --- | --- |
| 2026-08-23 | 4.5.21 candidate | Added W-126's explicit staff-only DEMO classification with default `false` and one fail-closed repreneur-eligibility veto. A classified opportunity and all lifecycle, match, pursuit, document, NDA and audit history remain available to staff, while every repreneur list, direct read, response, pursuit, confidential-resource, notification and Staff Portal Preview read or count is denied. Titles and references are never runtime classification logic. The additive schema migration performs no production classification; the separately confirmed 24-row manifest operation locks and rejects identity, timestamp, content-fingerprint or dependent-row-count drift before changing any row. | W-126; migration 112 candidate |
| 2026-08-23 | 4.5.22 candidate | Added W-128's deployable-but-inert one-time Draft activation boundary. Its read-only preflight returns ordered eligible and excluded rows with stable criterion fingerprints; its service-only apply accepts only a separately supplied matching manifest/digest, locks and revalidates the Active contract, updates only Draft status and audit actor, and retains a precise rollback manifest. It neither chooses a candidate set nor changes production data during migration. | W-128; migration 113 candidate |
