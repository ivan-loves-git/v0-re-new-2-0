...
| --- | --- | --- | --- |
| 2026-08-23 | 4.5.22 candidate | Added the separately authorised W-112 historical-pursuit apply candidate: a forced-RLS immutable source-row ledger, exact workbook/manifest binding, digest replay guard, staff-only history projection and rollback-only rehearsal runner. Exact resolved workbook rows may create only a non-confidential `draft` or terminal historical `dropped` match; existing status is preserved except that a clean draft may become dropped. External/missing rows remain ledger-only. It creates no journey evidence/event, Gate, NDA artifact, document, grant, notification, disclosure, completion or opportunity closure. | W-112 implementation candidate; migration 112 |
