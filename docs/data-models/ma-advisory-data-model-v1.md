...
15. A shared Acme provisional source context may be assigned only by staff. It may support `draft`, `active` and `paused` work while staff resolves the real source, but it cannot be used to close or archive an opportunity or to satisfy `cutover-ready` or `cutover-complete` treatment. Acme is never renamed into an actual intermediary; resolution changes the opportunity to the real office and retains immutable correction evidence.
...
| --- | --- | --- |
| Firm, office and named contact identities | Yes | Only for an active pursuit after both signed NDAs, staff Gate 2 and an explicit staff disclosure approval; access is revoked on NDA expiry, staff revocation or pursuit closure |
...
| Mutual interest | Repreneur; an eligible staff-assigned proposed match exists | Store the existing interest response and time; “Mutual interest” is computed from the two parties’ recorded responses | Staff validates the matching/pursuit preconditions before advancing | Staff only until the existing downstream disclosure gates permit otherwise | Retain response and source match/pursuit; append response event |
| E4 — staff prepares first external step | Staff; mutual-interest conditions and existing workflow prerequisites are met | Store the existing E4 state/evidence reference; “E4 ready/sent” is computed | Staff validates recipient, template or approved external step using the existing workflow | Staff only | Retain delivery/reference evidence; append E4 transition event |
| Intermediary qualification | Staff; source office and named contact are known or explicitly marked provisional | Store qualification decision and evidence reference in the existing relationship workflow; “Qualified” is computed | Staff records the operating basis; unresolved Acme status remains visible to staff | Staff only | Retain decision, basis and actor; append qualification event |
| Blank NDA handling | Staff initiates; eligible pursuit and correct party are identified | Store the existing blank-NDA document/evidence reference and its state; “Blank NDA available” is computed | Staff validates document type, party and pursuit linkage | Staff only | Retain document metadata and event; no repreneur visibility from blank NDA alone |
| Gate 1 | Staff; the existing pre-Gate-1 NDA and pursuit checks pass | Store Gate 1 decision, actor, time and evidence reference; “Gate 1 passed” is computed | Staff validation is mandatory | Staff only | Retain decision and evidence; append Gate 1 event |
| E6 — Re-New-signed pursuit copy | Staff initiates after Gate 1 and applicable document checks | Store existing pursuit-copy document/evidence and delivery state; “Re-New signed copy sent” is computed | Staff validates copy, recipient and delivery result | Staff only | Retain document and delivery evidence; append E6 event |
| Repreneur-signed pursuit copy | Repreneur submits through the existing confidentiality path after the approved request | Store signed-copy evidence, signer and received time; “Both signed NDAs” is computed only when both required copies validate | Staff validation of signer, document linkage and validity is mandatory | Staff only until Gate 2 and explicit disclosure approval also pass | Retain signed copy and validation outcome; append signed-copy receipt event |
| Gate 2 | Staff; both signed NDA copies are validated and the pursuit is active | Store Gate 2 decision, actor, time and evidence reference; “Gate 2 passed” is computed | Staff validation is mandatory | Staff only; Gate 2 alone does not disclose source identity | Retain decision and evidence; append Gate 2 event |
| Source identity disclosure approval | Staff; pursuit is active, both signed NDAs remain valid, and Gate 2 passed | Store explicit staff approval, scope and time; “Source identity visible” is computed from approval plus live NDA, Gate 2 and pursuit status | Staff validation is mandatory; no inferred approval is allowed | Repreneur may see only source firm, office and named contact for that pursuit | Retain approval and scope; append disclosure-approval event |
...
| E7 — post-Gate-2 external step | Staff; Gate 2 and the applicable workflow prerequisites pass | Store existing E7 state/evidence reference; “E7 ready/sent” is computed | Staff validates the permitted step and recipient | Staff only unless a separately approved item is repreneur-visible | Retain delivery/reference evidence; append E7 transition event |
...
| E8 — final workflow step | Staff; all existing E8 prerequisites pass | Store the existing E8 state/evidence reference; “E8 complete” is computed | Staff validates completion evidence | Role-appropriate; it does not widen source access by itself | Retain evidence; append E8 transition event |
...
| Close opportunity or pursuit | Staff; no unresolved provisional Acme source context and existing closure prerequisites pass | Store existing closed status, reason, actor and time; “Closed” is computed | Staff validation is mandatory; an Acme-linked opportunity cannot close | Staff only; source disclosure is revoked on pursuit closure | Preserve historical source/contact links and evidence; append closure event |
...
6. W-001 requires no data migration: it canonicalizes authority and acceptance criteria. Any future implementation must reuse the existing record/evidence paths or introduce an additive, audited mapping in this document and its migration, API, role-projection and test changes; it must not invent parallel fields.
...
| --- | --- | --- | --- |
| 2026-07-27 | 2.0 | Added the authoritative W-001 action-and-evidence matrix: staff/repreneur authority, explicit Gate 1/Gate 2 and source-disclosure conditions, lifecycle evidence/retention rules, and the shared Acme provisional-source correction and cutover-blocking rule. This documentation release introduces no migration or new user-facing capability. | W-001; W-021 decision; future Acme and W-010 implementation cards |
