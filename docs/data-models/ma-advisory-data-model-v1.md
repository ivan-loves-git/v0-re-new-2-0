...
| --- | --- | --- | --- |
| 2026-09-04 | 4.5.41 candidate | Ticket #94 makes REAL/DEMO selection explicit on every staff repreneur and opportunity creation surface, including External Pursuit conversion. Omission fails before persistence; candidate code uses a strict additive opportunity creator that atomically persists the Boolean selection and initial-classification actor/time, while the edit RPC rejects it. The legacy creator and conversion overload remain for ordered deploy and rollback compatibility. Active DEMO inventory remains usable only in the DEMO namespace. No existing row, match, pursuit, toggle, visibility rule or historical classification is changed. | GitHub #87, #88, #94; migration `117_explicit_demo_real_creation.sql` |
