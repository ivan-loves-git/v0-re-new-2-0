...
The W-061 foundation deliberately does not add an opportunity sourcing channel. Ivan rejected that proposal. `opportunities.repreneur_exposure` remains a compatibility field rather than a source-disclosure authority, but W-021 now governs its bounded use as the broad Deal Flow publication control while the current portal reads depend on it. The W-061 service continues to write `staff_only` for every new record and every transition from or to `draft` as the intake compatibility firewall; it does not backfill or alter an already visible active record. W-063 remains the staff application boundary for lifecycle: activation never publishes an opportunity. Only the separate W-021 staff publication action may change an eligible Active non-DEMO record from `staff_only` to `anonymized`, or withdraw that broad publication back to `staff_only`. The legacy `repreneur_visible` value is not written by this operating rule.
...
| `is_demo` | Boolean | Always; defaults to `false` | Staff only | WAVE | Explicit staff classification. `true` is an unconditional repreneur-eligibility veto; display text is never classification logic |
...
| `repreneur_exposure` | `staff_only`, `anonymized`, legacy `repreneur_visible` | Always; defaults to `staff_only` | Staff only | WAVE | Compatibility control for unmatched broad Deal Flow discovery only. W-021 writes `anonymized` after explicit staff publication approval and may withdraw it to `staff_only`; lifecycle never publishes automatically and no new action writes the legacy value |
...
4. Repreneur disclosure is a separate downstream decision. An anonymized opportunity projection additionally requires location, sector, public title and teaser summary plus either an exact visible match or explicit W-021 staff approval for broad Deal Flow publication. Source identity, documents and other confidential material still require their existing pursuit-specific gates.
...
6. Missing financial values remain `null`. Matching v2 never converts a missing opportunity value into zero or a fit; it returns an explicit review reason and applies the evidence cap defined below. A missing buyer minimum EBITDA margin alone uses the centrally versioned provisional 0% scoring fallback without writing 0 back to the profile; its score reason says that fallback was used.
...
11. Staff create and edit use the atomic office-context services only. They do not write `source_id`, `source_label`, `opportunity_source_contacts`, `repreneur_exposure`, an origin channel or a separate source-contact relationship. W-021 publication is a separate narrow action and never rides on a create, edit or lifecycle transition.
...
23. `repreneur_geography_targets` is the staff-only France-tree identity used by Matching v2. The database synchronizes it atomically from exact supported `q12_geo_zones`/`target_location` values on future profile writes; migration 092 performs no big-bang backfill of current profiles. The matcher loads each selected node and its ancestors. Equal or ancestor coverage is a fit; an opportunity broader than a narrower target is review; known disjoint branches are demoted. If an existing profile has no synchronized bridge row, or either side is not mapped, geography is review rather than a fuzzy or custom-label inference. The bridge is never exposed to repreneurs.
24. W-073 keeps the shared Acquisition Project/Target Thesis on the canonical `repreneurs` record. A repreneur may update their own `q12_geo_zones`, `q13_target_sectors_v2`, `q14_deal_size`, `q16_equity`, target revenue range, minimum EBITDA margin and target staff-size range; staff may correct those same fields only through the staff-guarded action for that repreneur. Both paths validate the existing taxonomy and ranges and recompute Matching v2 score and reason snapshots. WHO, WHEN, scoring flags and the broad deal-size answer remain qualification or compatibility context only; they do not contribute to signed-client matching. Automatic staff candidate lists and repreneur unassigned-deal discovery require lifecycle `client` plus an accepted paid Deal Flow or End-to-End offer; free and test-marked profiles are excluded. A staff-proposed match and its history remain visible regardless of this automatic-discovery entitlement. Recalculation never changes a human recommendation, match status, note or pursuit state.
25. W-070 gates only automatic unassigned deal-flow discovery, not a staff-proposed match, pursuit history or staff matching tools. Automatic discovery requires a canonical-or-supported-legacy geography selection, a canonical-or-supported-legacy sector selection, and at least one detailed numeric target: a revenue bound, minimum EBITDA margin or headcount bound. WHO, WHEN, scoring flags and the broad deal-size answer do not satisfy or block this gate. While incomplete or not entitled to the matching service, the repreneur sees staff-selected opportunities; direct access to an otherwise unassigned deal returns no deal.
26. Matching v2 provisional configuration `provisional-2026-08-23` uses sector 30%, geography 25%, revenue 25%, minimum EBITDA margin 12% and headcount 8%, with a buyer-margin fallback of 0%. Revenue and headcount bounds are inclusive; a value up to 20% outside a known boundary receives half of that criterion and requires staff judgement. Opportunity EBITDA margin is calculated as `ebitda_keur / (revenue_meur * 1,000) * 100`; meeting the minimum is a fit and being no more than two percentage points below it receives half credit and review. Scores normalize across known evidence, but any review outcome caps the score at 70. A known sector mismatch caps the score at 44, sector review at 70, known disjoint geography at 60, geography review at 70, a clear revenue or EBITDA-margin mismatch at 44, and a clear headcount mismatch at 64. Staff-pushed opportunities remain visible and authoritative regardless of the automated score. Missing and invalid opportunity inputs are review, never a silent fit or zero-equivalent. These values are deliberately centralized: Bertrand's confirmed calibration changes this named configuration and its representative tests, not the algorithm or data contract. Ongoing correlation analysis belongs to a separate M3 calibration card and does not keep W-008 open after release.
...
28. W-126 makes `is_demo = true` a fail-closed veto across every repreneur list, browse, match, direct detail, response, active-pursuit, confidential document, NDA, notification and portal-preview read or count. Staff alone classifies or unclassifies a row through the dedicated control; unclassification requires explicit confirmation because ordinary visibility and confidentiality rules may make the row eligible again. Classification changes no lifecycle, match, pursuit, document, NDA, disclosure evidence or retention record, and staff operational views retain the complete record with a clear DEMO indicator. Titles, references, source labels and other display text are candidate-review evidence only and never runtime logic. Existing production rows remain `false` until staff separately confirms the pinned exact manifest; that backfill locks and rejects identity, fingerprint, timestamp or dependent-row cardinality drift before changing any row.
29. W-021 keeps lifecycle and unmatched broad Deal Flow publication separate. Every new opportunity and every transition from or to `draft` remains `staff_only`. Staff may publish only an `active`, non-DEMO opportunity with non-blank `public_title`, `teaser_summary`, `sector` and `location` that still satisfies the valid source-office and contact contract. The intermediary firm's `prospect` or `active` commercial CRM status is not a publication criterion; an active source office and valid active contact context are. Publication writes only `anonymized`; it never writes `repreneur_visible`, creates a match, changes a pursuit or unlocks source identity, a source teaser, NDA or Information Memorandum.
30. The W-021 staff publication action is explicit and audited. Publishing changes `staff_only` to `anonymized`; withdrawing broad publication changes `anonymized` to `staff_only`. Withdrawal does not hide an exact current proposed, interested, declined, active-pursuit or dropped match from its owner and does not alter that match or pursuit. DEMO, Draft, Paused, Closed, Archived and legacy-`repreneur_visible` records fail closed through this action. An incomplete but still Active, non-DEMO published record may be withdrawn immediately; incompleteness must never trap an unsafe teaser in broad discovery.
...
12. A staff-only draft may have no source office or contacts. Moving to `active` or `paused` happens through the atomic office-context service and requires the full valid-opportunity rules. Activation never broadens repreneur disclosure; the exposure field stays `staff_only` for new records and draft transitions until staff separately uses the W-021 publication action.
...
| Opportunity public title, teaser, sector, location and approved metrics | Yes | The exact repreneur may see the anonymized opportunity through their current proposed, interested, declined, active-pursuit or dropped match. The server computes one exclusive Deals placement after authorization: `recommended` for proposed, `in_progress` for interested or active pursuit, `declined` for declined or dropped, otherwise `live` only through broad discovery. A dropped match stays anonymized history and restores no confidential access. Broad Deal Flow discovery requires an eligible Active non-DEMO opportunity plus explicit W-021 staff publication approval recorded as `anonymized`. |
...
`opportunities.repreneur_exposure` is the current broad-discovery compatibility filter, not a source-disclosure or lifecycle authority. W-021 gives only two values current operating meaning: `staff_only` excludes an unmatched opportunity from broad discovery, while `anonymized` records explicit staff publication approval for a complete Active non-DEMO teaser. The legacy `repreneur_visible` value remains readable but is not written by the W-021 action. Exposure must not hide an active opportunity from the repreneur who owns its current visible `opportunity_matches` row; that row authorizes only the anonymized opportunity projection, never source identity, internal description, notes, documents or confidential access.
...
| `opportunities.repreneur_exposure` | Current W-021 broad-discovery compatibility control, separate from W-061 intake, lifecycle and confidential disclosure. The atomic intake service writes `staff_only` for new records and draft transitions. A separate audited staff action may publish a complete Active non-DEMO teaser as `anonymized` or withdraw it to `staff_only`; exact matches and pursuit-specific gates remain independent |
...
| --- | --- | --- | --- |
| 2026-08-26 | 4.5.31 candidate | Corrected W-021 publication eligibility so an intermediary firm's commercial CRM status cannot hide an otherwise complete opportunity. Prospect firms may carry active source offices and current opportunities; the active-office, valid-contact and complete anonymized-teaser checks remain fail-closed. The one-time publication still requires the exact fresh 111-row manifest and changes only broad exposure. | W-021 corrective publication candidate; migration `20260826142358` |
