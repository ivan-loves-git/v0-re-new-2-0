...
| Implementation status | Migrations 076 to 078 were applied to production and schema-verified on 2026-07-26. The canonical office/contact model is live and the one-time cutover staging area is empty; no real workbook has been imported and Excel remains operationally authoritative until W-010. Migration 079 is the checked-in, not-yet-production-verified W-064 provisional Acme foundation: one Acme Co. / Acme Paris context, immutable assignment and resolution evidence, and service-only lifecycle/cutover guards. The W-010 date-precision and test-data replacement rules remain approved target gaps and must be implemented and production-verified before the real workbook is staged. |
...
| `ma_provisional_source_contexts`, `ma_provisional_source_review_events` | Migration 079 provisions exactly one shared Acme Co. / Acme Paris context with Bertrand's existing canonical contact, plus immutable assignment/resolution snapshots | Keep Acme as provisional operational context only. Compute review-required from canonical source office plus unresolved append-only evidence; block opportunity close/archive and cutover treatment until resolution. The service-only resolution primitive is for W-065; no review UI is included here. | Checked in; requires disposable rehearsal and production verification before live status |
...
| --- | --- | --- | --- |
| 2026-07-27 | 2.1 | Added the checked-in W-064 Acme provisional-source foundation: deterministic Acme Co. / Acme Paris provisioning with the existing Bertrand canonical contact, append-only assignment and resolution evidence, computed review-required, service-only assignment/resolution primitives, and deferred opportunity/cutover guards. No staff review UI, repreneur projection or external email route is included. | W-064 and migration 079 |
