...
3. The read-only input snapshot is the deterministic JSON shape `{ repreneurs: [{ id, first_name, last_name }], opportunities: [{ id, reference }], matches: [{ opportunity_id, repreneur_id }] }`. The current approved preflight expectation is exactly 46 resolved records: 33 proposed staff-only draft creates, 13 existing-pair merges, and 14 `none` unresolved rows. A changed count fails rather than producing a partial apply candidate.
...
6. No candidate action creates canonical pursuit evidence/events, Gate 1 or 2, NDA artifacts, documents, grants, emails, notifications, source disclosure, completion or opportunity closure. It may not use `opportunity_pursuit_events` as an import ledger because that table has no idempotency key or unique source-row constraint. `scripts/run-historical-pursuit-import.mjs --rehearse` calls the controlled database function within one transaction and always rolls it back; `--apply` exists only for the separately approved production implementation card.
...
| --- | --- | --- | --- |
| 2026-08-23 | 4.5.22 candidate | Added the separately authorised W-112 historical-pursuit apply candidate: a forced-RLS source-row ledger, digest replay guard and rollback-only rehearsal runner. Exact resolved workbook rows may create or merge only a non-confidential `proposed`, `interested` or historical `dropped` match; external/missing rows remain ledger-only. It creates no journey evidence/event, Gate, NDA artifact, document, grant, notification, disclosure, completion or opportunity closure. | W-112 implementation candidate; migration 112 |
