...
| --- | --- | --- | --- |
| 2026-08-29 | 4.5.34 candidate | Added W-170's narrowly guarded unused-retained-document correction. Staff may remove only an exact unused Information Memorandum or latest unused canonical NDA version through one locked service-role boundary; source teasers, cross-opportunity requests, legacy links, pursuit grants, validation/Gate evidence and superseded versions fail closed. Metadata is removed before private Storage cleanup, leaving a server-only retry receipt rather than a broken live reference if Storage is temporarily unavailable. | W-170; migration 117 candidate |
