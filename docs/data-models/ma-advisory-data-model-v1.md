...
22. `repreneur_geography_targets` is a staff-only compatibility bridge for exact France-first questionnaire values. The database synchronizes it atomically from `q12_geo_zones`/`target_location` only on future profile writes; migration 092 performs no big-bang backfill of current profiles. Unknown or foreign legacy values stay legacy-only, current canonical-target adoption remains a separate controlled and verified operation, and this release does not change Matching v2 behavior or expose the bridge to repreneurs.
23. W-073 keeps the shared Acquisition Project/Target Thesis on the canonical `repreneurs` record. A repreneur may update their own `q12_geo_zones`, `q13_target_sectors_v2`, `q14_deal_size`, `q16_equity`, target revenue range, minimum EBITDA margin and target staff-size range; staff may correct those same fields only through the staff-guarded action for that repreneur. Both paths validate the existing taxonomy and ranges, preserve the compatibility fields used by the current matcher, recompute the existing scores and stored match snapshot, and never change a human recommendation or match status.
24. W-070 gates only automatic unassigned deal-flow discovery, not a staff-proposed match, pursuit history or staff matching tools. The one completeness rule uses exactly the six dependencies consumed by the current scorer: stored WHO score, stored WHEN score, stored scoring flags (an empty array is valid), and a canonical-or-legacy selection for geography, sector and deal size. Revenue, EBITDA margin and staff-size preferences remain optional context and cannot suppress discovery by themselves. While incomplete, the repreneur sees staff-selected opportunities and an edit link; direct access to an otherwise unassigned deal returns no deal. This is a confidence gate around the existing scorer, not Matching v2 or a scoring change.
25. W-126 makes `is_demo = true` a fail-closed veto across every repreneur list, browse, match, direct detail, response, active-pursuit, confidential document, NDA, notification and portal-preview read or count. Staff alone classifies or unclassifies a row through the dedicated control; unclassification requires explicit confirmation because ordinary visibility and confidentiality rules may make the row eligible again. Classification changes no lifecycle, match, pursuit, document, NDA, disclosure evidence or retention record, and staff operational views retain the complete record with a clear DEMO indicator. Titles, references, source labels and other display text are candidate-review evidence only and never runtime logic. Existing production rows remain `false` until staff separately confirms the pinned exact manifest; that backfill locks and rejects identity, fingerprint, timestamp or dependent-row cardinality drift before changing any row.
...
| 2026-08-23 | 4.5.24 candidate | Added W-128's deployable-but-inert one-time Draft activation boundary. Its read-only preflight returns ordered eligible and excluded rows with stable criterion fingerprints; its serialized service-only apply accepts only a separately supplied manifest/digest equal to the complete fresh eligible set, locks and revalidates the Active contract, updates only Draft status and audit actor, and rejects any second run. It neither chooses a candidate set nor changes production data during migration. The guarded rollback records exact post-activation audit values plus a content-free downstream lifecycle digest, rejects opportunity or linked match/pursuit/document drift, and writes immutable evidence; service role has read-only audit-table access and truncate is guarded. | W-128; migration 115 candidate |
