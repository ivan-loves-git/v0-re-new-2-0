...
3. The read-only input snapshot is the deterministic JSON shape `{ repreneurs: [{ id, first_name, last_name }], opportunities: [{ id, reference }], matches: [{ opportunity_id, repreneur_id }] }`. The current approved preflight expectation is exactly 46 resolved records: 33 proposed `create_draft_match_with_historical_staff_note`, 13 proposed `merge_historical_staff_note`, and 14 `none` unresolved rows. A changed count fails rather than producing a partial apply candidate.
4. A resolved manifest record contains a staff-only historical proposal summary and its exact current IDs for review. Its proposed create action is a future **draft match plus staff-only historical note** only; its proposed merge action may append the same note only after source-row idempotency is database-enforced. Neither is implemented or authorized by this preflight. No proposed action contains a current pursuit, gate, NDA/artifact, document, disclosure or opportunity-state mutation. Any future import needs a separately authorized production card, an approved resolved manifest digest, a transactionally idempotent database-owned evidence model, a full rehearsal and explicit treatment of existing matches. It may not use `opportunity_pursuit_events` as an import ledger because that table has no idempotency key or unique source-row constraint.
5. The preflight creates the output path once. A changed workbook, incomplete snapshot, count drift or second attempt to overwrite a manifest fails closed. The output is staff-only operational evidence and must not be sent to or surfaced for repreneurs.

## Change log

| Date | Version | Change | PDR or implementation reference |
| --- | --- | --- | --- |
| 2026-08-23 | 4.5.21 candidate | Added W-112's source-hash-bound, no-write historical-pursuit workbook mapping preflight. It resolves only exact current buyer and Re-New reference pairs into staff-review facts and fails closed for external, missing or ambiguous rows; inconsistent terminal/reason labels remain explicit review flags. Workbook stage labels are retained as historical evidence only: they cannot create a match, pursuit, Gate, NDA artifact, document, memo, disclosure, event or opportunity outcome. Any later apply path requires its own approved card, manifest digest, database-owned idempotency and rehearsal. | W-112 evidence mapping candidate |
