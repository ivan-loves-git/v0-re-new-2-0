...
7. Repreneur template download requires an active pursuit and resolves the exact current `blank_template` bound to current Gate 1 in one canonical database operation. A dropped pursuit, an inactive opportunity or a superseding template without fresh validation returns no downloadable document; browser projections and UI visibility are never download authority.
