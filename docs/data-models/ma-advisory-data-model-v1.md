...
| --- | --- | --- | --- |
| 2026-09-04 | 4.5.41 candidate | Ticket #94 makes REAL/DEMO selection explicit on every staff repreneur and opportunity creation surface, including External Pursuit conversion. Omission fails before persistence; the canonical opportunity creator requires and atomically persists the Boolean selection, with additive initial-classification actor/time attribution, while the edit RPC rejects it. Active DEMO inventory remains usable only in the DEMO namespace. No existing row, match, pursuit, toggle, visibility rule or historical classification is changed. | GitHub #87, #88, #94; migration `117_explicit_demo_real_creation.sql` |
