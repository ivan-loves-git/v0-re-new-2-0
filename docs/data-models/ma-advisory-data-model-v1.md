...
### W-130 staff correction boundary (implementation candidate)

Staff may correct only the following existing canonical fields through the three typed, service-role-only correction services: firm `name`, `category`, `network_label`, `website_url`, `internal_notes`; office `name`, `city`, `address`, `coverage_note`, `website_url`, `general_email`, `general_phone`, `internal_notes`; contact `first_name`, `last_name`, `email`, `phone`, `linkedin_url`, `internal_notes`; and `job_title` on the selected existing office affiliation only. Each call requires an actor, locks the exact row and normalized identity key where appropriate, and retains `updated_at`/`updated_by` audit; migration 114 adds nullable affiliation `updated_by` solely for the job-title correction actor. The canonical Contacts workspace shows the exact affiliation and office before job-title correction. Calls trim optional text, validate URL and email formats, and reject normalized active firm or same-firm office duplicates without merging records. A contact email is a review signal, not a global identity constraint. An Active or Paused opportunity cannot lose a usable primary contact email in the same transaction.

These services do not accept a generic patch and cannot create, end, move or reactivate an affiliation; change firm or office ownership; alter status, archive or delete records, `is_default`, geography, coverage regions, primary or source links, campaign suppression, source evidence or disclosure; or create a repreneur projection. Existing lifecycle history and source confidentiality therefore remain unchanged.
