...
Staff may correct only the following existing canonical fields through the three typed, service-role-only correction services: firm `name`, `category`, `network_label`, `website_url`, `internal_notes`; office `name`, `city`, `address`, `coverage_note`, `website_url`, `general_email`, `general_phone`, `internal_notes`; contact `first_name`, `last_name`, `email`, `phone`, `linkedin_url`, `internal_notes`; and `job_title` on the selected existing office affiliation only. Each call requires an actor, locks the exact row (and normalized identity key where appropriate), retains normal `updated_at`/`updated_by` audit; migration 113 adds nullable affiliation `updated_by` solely for the job-title correction actor. The canonical Contacts workspace shows the exact affiliation/office before job-title correction. Calls trim optional text, validate URL/email formats, and reject normalized active firm or same-firm office duplicates without merging records. A contact email is a review signal, not a global identity constraint. An Active or Paused opportunity cannot lose a usable primary contact email in the same transaction.

These services do not accept a generic patch and cannot create, end, move or reactivate an affiliation; change firm or office ownership; alter status, archive/delete records, `is_default`, geography/coverage regions, primary/source links, campaign suppression, source evidence or disclosure; or create a repreneur projection. Existing lifecycle history and source confidentiality therefore remain unchanged.
...
| --- | --- | --- | --- |
| 2026-08-23 | 4.5.21 candidate | Added W-130's narrow staff-only correction boundary: typed service-role RPCs permit only the approved firm, office, contact and selected-affiliation job-title fields, with actor/timestamp audit including the affiliation-level `updated_by`, normalized duplicate rejection, input validation and active/paused-primary-email protection. The Contacts workspace carries the exact selected affiliation context for a job-title correction. The services cannot move or end relationships, change lifecycle/default/geography/source/suppression/disclosure fields, merge records or create a repreneur projection. | W-130; migration 113 candidate |
