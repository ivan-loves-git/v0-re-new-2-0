...
16. A blank source-workbook platform title never clears a non-blank WAVE `public_title`. Missing public title does not block lifecycle publication: the portal uses the neutral non-identifying label `Confidential acquisition opportunity` and never falls back to the internal reference, description or source identity. W-163's release-candidate operator accepts only the digest-pinned reviewed workbook, reads non-blank `Titre sur plateforme` values keyed by unique `Ref. Mandat`, and changes only `public_title`. It first reports aggregate no-op, guarded-update, conflict and blocker counts; an apply requires the exact digest and compare-and-set match on the preflight title, rejects DEMO, missing and ambiguous references, and rolls back the entire run on drift. The workbook and any title values remain outside Git and operator output.
...
17a. The approved Colin Deal Flow correction uses an exact 130-row, activity-first sector manifest for the current Active REAL inventory. It writes only canonical labels from the 16-label taxonomy, retains before-images in the existing immutable sector-taxonomy ledger, and fails closed if the scoped inventory, manifest or canonical geography nodes drift. The three related geography corrections set AU-001 to `Auvergne-Rhône-Alpes` and retain/fill `Grand-Ouest` on GO-020 and GO-021. Its operator is dry-run by default and has a fail-closed rollback that refuses to overwrite a later staff edit. It neither changes DEMO opportunities nor creates a source identity.
17b. AU-001, Idf-001 and PA-005 have verified synthetic-DEMO source chains attached to REAL opportunities. Until a real intermediary is verified, the database source graph and its historic provenance remain untouched. The guarded operator sets a generic persisted `source_identity_to_verify` flag only on those reviewed opportunity references. Staff-only projections render it as `Source identity to verify`; the authoritative source-review predicate blocks the same source-dependent operations, and repreneur projections expose neither the flag nor source identity. No Acme or other provisional identity is inferred for those records.
...
28. W-126 introduced explicit `is_demo` classification and production exclusion. W-164 refines its portal meaning into an isolated test namespace: DEMO-to-DEMO reads and actions are valid for controlled UAT, while every REAL-to-DEMO or DEMO-to-REAL pair fails closed. Staff alone classifies or unclassifies a row through the dedicated control; matched records cannot be reclassified without a separate approved relationship treatment. Classification changes no lifecycle, match, pursuit, document, NDA, disclosure evidence or retention record, and staff operational views retain the complete record with a clear DEMO indicator. Titles, references, source labels and other display text are candidate-review evidence only and never runtime logic. Ticket #94 requires a visible, deliberate REAL or DEMO selection on every staff-created repreneur, opportunity and External Pursuit conversion; omission fails before persistence. Candidate opportunity creation uses the additive strict `create_opportunity_with_office_context_v2` JSONB payload: it requires `is_demo`, strips it before the legacy writer, and writes the chosen value plus initial classification actor/time before commit. The edit RPC rejects `is_demo`; no default, backfill or historical reclassification is introduced. Migration 117 temporarily retains the legacy creator and seven-argument conversion solely for ordered deployment and rollback; after the candidate application is deployed and verified, migration 118 removes those public service endpoints, retains inaccessible migration-private implementations only for the paired database-first rollback, and leaves v2 as the only creation endpoint.
...
| `opportunities.is_demo` | W-126 adds a non-null Boolean with database default `false`; Ticket #94 requires every candidate staff creation path, including External Pursuit conversion, to provide an explicit Boolean through the strict additive creator in the same transaction | Preserve the opportunity and every linked lifecycle, match, pursuit, document, NDA and audit record for staff. Existing rows remain unchanged; only the dedicated staff control or separately reviewed treatment may classify an existing row. Every repreneur authorization predicate fails while it is true. Migration 118 removes the legacy omission-capable public creator and seven-argument conversion after candidate deploy; database rollback precedes application rollback if restoration is required. | W-126 corrected integrated release candidate; Ticket #94 migrations `117_explicit_demo_real_creation.sql` and `118_ticket_94_strict_creation_cutover.sql` |
...
| --- | --- | --- | --- |
| 2026-09-04 | 4.5.41 candidate | Ticket #94 makes REAL/DEMO selection explicit on every staff repreneur and opportunity creation surface, including External Pursuit conversion. Omission fails before persistence; candidate code uses a strict additive opportunity creator that atomically persists the Boolean selection and initial-classification actor/time, while the edit RPC rejects it. Migration 117 retains the legacy creator and conversion overload only through ordered deploy; post-deploy migration 118 removes those omission-capable public service endpoints. Its paired rollback restores database compatibility before the prior application, avoiding an endpoint outage. Active DEMO inventory remains usable only in the DEMO namespace. No existing row, match, pursuit, toggle, visibility rule or historical classification is changed. | GitHub #87, #88, #94; migrations `117_explicit_demo_real_creation.sql`, `118_ticket_94_strict_creation_cutover.sql` |
| 2026-09-02 | 4.5.40 candidate | Added the dry-run-first, exact-manifest Colin Deal Flow correction: it fails closed unless the live inventory is exactly 155 Active REAL opportunities (130 legacy-sector corrections plus 25 already canonical), then maps the 130 to the 16-label taxonomy with retained before-images; AU-001, GO-020 and GO-021 receive exact canonical geography/display corrections; and a non-reversible source-chain digest permits a generic, staff-only `Source identity to verify` flag only on the three approved references. The source graph is preserved, no replacement intermediary is invented, the source-review database predicate blocks source-dependent operations, and the rollback refuses any later staff edit. The accompanying additive migration enables RLS without policies, grants or FORCE RLS on the five W-021/W-128 audit tables. | GitHub #74, #80 and #82; migration `20260902202154_colin_data_security_corrections.sql` |
| 2026-08-29 | 4.5.39 candidate | Added W-163's digest-pinned, dry-run-first reviewed platform-title operator. It accepts only the exact one-sheet reference/title source shape, preserves blank source cells, rejects duplicate, missing, ambiguous or DEMO targets, and applies only `public_title` through a per-row compare-and-set transaction that rolls back entirely on drift. Aggregate evidence contains no source values. Find and Groups now lead staff scanning with Title, Region, Sector, Revenue and EBITDA while keeping reference subordinate and journey/pursuit context accessible. | W-163 / GitHub #35 candidate |
| 2026-08-29 | 4.5.38 released | Scoped W-169's internal Pause transition flag to the exact guarded update. A dedicated Pause call now restores the caller's prior flag before it appends history, so a direct service transaction cannot inherit authority and pause a second opportunity. Production migration `20260829203000` is live exactly once; full-schema rehearsal, rollback-only production proof and desktop/mobile staff browser QA passed on release `835affed`. | W-169; migration `20260829203000_w169_pause_guard_scope.sql`; PR #83; GitHub Product Change #19 and Ticket #22 |
| 2026-08-29 | 4.5.37 released | Added W-169's staff-only lifecycle outcome separation. Future opportunity closure accepts only permanent deal-wide reasons; Paused by cabinet moves an Active opportunity to Paused through one append-only reason record; No viable match and repreneur-specific due-diligence failure drop only the selected pursuit and persist the exact reason. Database guards reject generic pause and cross-category future writes. Existing closure enum values, the historical Paused by cabinet closure and all reasonless historical Drop evidence remain unchanged. Production migration `20260829180000` is live exactly once with no backfill or historical rewrite. | W-169; migration `20260829180000_w169_lifecycle_outcome_separation.sql`; PR #82; GitHub Product Change #19 and Ticket #22 |
