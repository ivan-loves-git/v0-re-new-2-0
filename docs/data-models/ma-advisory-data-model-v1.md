...
| `date_added_precision` | `day`, `month` | Conditional | Staff only | WAVE | Required for date-aware writes; preserves whether the source supplied a full calendar date or only month and year. Pre-W-098 legacy dates may remain `null` when source precision was not reconciled; WAVE must not invent it. |
...
13. `date_added` and `date_added_precision` are both null when no source date is known. A known full calendar date uses precision `day`. A pre-W-098 legacy date with unreconciled source precision may retain `null` precision; it is not silently classified.
14. A source value containing only month and year is stored technically as the first day of that month with precision `month`. The artificial day must never be displayed, exported, described as an event day or used for day-level age or service-level reasoning. W-098 corrects only six source-hash-bound legacy rows and never generalizes this repair into an importer.
...
W-010 defined the target date-aware cutover boundary. W-098 supplies the missing additive precision column only for its six source-hash-bound month-only repairs; all other existing legacy values remain precision-unknown until separately source-reconciled. Later staff intake or correction must write or clear a known pair atomically; no ordinary caller may normalize a month-only value through the earlier date-only allowlist.
...
| `opportunities.date_added_precision` | W-098 adds the controlled `day`/`month` companion column and sets `month` only for six hash-bound legacy CRM corrections. Other pre-existing non-null dates remain precision-unknown rather than guessed | Preserve known month precision through display, export and later correction; never present the technical first day as a source event day | W-098 migration candidate; production application remains separately approved |
