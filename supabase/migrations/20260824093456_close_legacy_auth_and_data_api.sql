...
BEGIN
  ...
  END LOOP;
  FOR item IN SELECT schemaname, tablename, policyname FROM pg_policies WHERE schemaname = 'public' AND roles && ARRAY['anon'::name, 'authenticated'::name] AND tablename NOT IN ('clipboard', 'pdr_feedback', 'pdr_goals', 'pdr_milestones', 'pdr_proposals', 'pdr_requests', 'pdr_work_cards') LOOP
