...
BEGIN
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(payload->>'initiated_by_user_id', 0));
  IF (SELECT count(*) FROM public.ai_generation_runs AS runs WHERE runs.initiated_by_user_id = payload->>'initiated_by_user_id' AND runs.started_at >= window_started_at) >= request_limit THEN
  ...
  ) RETURNING public.ai_generation_runs.generation_id, public.ai_generation_runs.trace_id, public.ai_generation_runs.started_at;
END $$;
