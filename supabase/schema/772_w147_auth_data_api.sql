...
BEGIN
  ...
  END LOOP;
  FOR item IN SELECT schemaname, tablename, policyname FROM pg_policies WHERE schemaname = 'public' AND roles && ARRAY['public'::name, 'anon'::name, 'authenticated'::name] AND (tablename NOT IN ('clipboard', 'pdr_feedback', 'pdr_goals', 'pdr_milestones', 'pdr_proposals', 'pdr_requests', 'pdr_work_cards') OR cmd <> 'SELECT') LOOP
...
-- Hosted Supabase creates customer-owned application objects as postgres.
-- supabase_admin is provider-internal and is not assumable by the customer
-- migration role, so release preflight must reject any different app owner.
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE ALL ON TABLES FROM PUBLIC, anon, authenticated;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE ALL ON SEQUENCES FROM PUBLIC, anon, authenticated;
-- PostgreSQL grants EXECUTE on new functions to PUBLIC globally. A
-- schema-scoped default REVOKE cannot override that built-in global grant.
