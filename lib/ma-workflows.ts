...
export async function sendMaSourceWorkflowEmailPayload(
  ...
  let handoffAttempt: HandoffAttempt | undefined
  if (handoff) {
    try {
      ...
      if (handoffAttempt.delivery_status === "in_flight") {
        await releaseReservation()
        return { success: false, message: "This handoff is still in flight. Retry the unchanged handoff in two minutes.", operationState: "pending" }
      }
    } catch (error) {
      await releaseReservation()
      return { success: false, message: error instanceof Error ? error.message : "The current handoff could not be verified." }
    }
  }
  ...
  const { data: pendingRows, error: beginError } = await supabase.rpc(
    "begin_ma_interaction_email_send",
    {
      ...
      p_client_operation_key: handoffAttempt?.operation_key ?? clientOperationKey,
  ...
  if (existingDeliveryStatus === "sent") {
    ...
    try { eventId = await finishHandoff("sent", interactionId, pendingInteraction.provider_message_id, null) }
    catch { await releaseReservation(); return { success: false, message: "Delivery was accepted; retry this unchanged handoff after two minutes to finish its evidence.", operationState: "pending" } }
    await releaseReservation()
    return {
      success: true,
      message: `Email was already sent to ${recipientEmail}`,
      operationState: "sent", eventId,
    }
  }
  if (existingDeliveryStatus === "failed") {
    try { await finishHandoff("failed", interactionId, null, existingDeliveryError) }
    catch { await releaseReservation(); return { success: false, message: "The rejection evidence is pending. Retry this unchanged handoff in two minutes.", operationState: "pending" } }
    await releaseReservation()
    return {
      success: false,
      message:
  ...
  try {
    if (handoff) await assertPursuitHandoffCurrent(supabase, handoff)
  ...
  let handoffEventId: string | undefined
  try {
    handoffEventId = await finishHandoff(result.outcome, interactionId, result.outcome === "sent" ? result.providerMessageId : null, result.outcome === "failed" ? result.error : null)
  } catch {
    await releaseReservation()
    return { success: false, message: "Delivery is recorded; retry the unchanged handoff after two minutes to complete its evidence.", operationState: "pending" }
  }
  ...
  return {
    ...
    operationState: "sent", eventId: handoffEventId,
  }
}
