...
type ProposalRow = {
  ...
  created_by: string; requester_actor: string; requester_user_id?: string | null; requester_display_name?: string | null
...
function projectRequest(row: RequestRow): PdrHistoryRequest {
  return { id: row.id, title: row.title, originalText: row.description, conversation: asArray(row.challenge_prompts), screening: { proposalType: "legacy_request", problemStatement: row.title, aiRationale: `Challenge score: ${row.challenge_score}`, status: row.status }, requester: { displayName: null, userId: null, actor: "", legacyLabel: row.created_by }, disposition: { kind: null, byUserId: null, at: null, note: row.decision_note }, createdAt: row.created_at, updatedAt: row.updated_at, attachments: [], provenance: "request" }
}
...
export async function listPdrRequestHistory(): Promise<PdrHistoryRequest[]> {
  ...
  const { data, error } = await supabase.from("pdr_proposals")
    .select("id, original_text, conversation, proposal_type, problem_statement, ai_rationale, status, created_by, requester_actor, requester_user_id, requester_display_name, disposition_kind, disposition_by_user_id, disposition_at, reviewer_note, created_at, updated_at")
