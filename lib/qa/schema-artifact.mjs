...
export function schemaObjectInventory(source) {
  return [...source.matchAll(/^-- Name: (.*); Type: ([^;]+); Schema: public;/gm)]
    .map((match) => `${match[2]}:${match[1]}`)
    .filter((value) => !value.startsWith("ACL:") && !value.startsWith("DEFAULT ACL:"))
    .sort()
}
