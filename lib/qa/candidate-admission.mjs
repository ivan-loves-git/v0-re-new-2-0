...
export async function validateCandidateContractAdmission({ eventName, trustedRoot, candidateRoot, dispatch = {} }) {
  ...
  if (eventName === "workflow_run") {
    ...
    if (!candidate.content.equals(trusted.content) || candidate.contract.files.length !== trusted.contract.files.length || !filesMatch) {
      fail("automatic-trust")
    }
    return { admission: "trusted-main-identical", contractSha256: candidate.contractSha256 }
  }
  if (eventName !== "repository_dispatch") fail("event")
  if (dispatch.schemaReviewed !== true || dispatch.schemaReviewVersion !== REVIEW_VERSION) fail("dispatch-schema-review")
  ...
  const filesMatch = trusted.contract.files.every((file) => candidate.files.get(file.path)?.equals(trusted.files.get(file.path)))
  if (candidate.content.equals(trusted.content) && candidate.contract.files.length === trusted.contract.files.length && filesMatch) {
    fail("dispatch-no-schema-change")
  }
  return {
