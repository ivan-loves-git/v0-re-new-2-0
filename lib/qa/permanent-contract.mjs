...
const QA_ALIAS = "renew-overnight-validation-git-59fa20-myworkmail4-pngs-projects.vercel.app"
const TIER_3_AUTHORIZER = "ivan-loves-git"
...
export function assertDeployedQaContract(expected, actual, { expectedOrigin = stableQaOrigin() } = {}) {
  if (actual?.origin !== expectedOrigin) fail("origin")
...
export function assertAuthorizedCandidate({ controllerRef, controllerSha, mainSha, actor, runAttempt, actorPermission, pull, verifyCheck, candidateBranch, candidateSha }) {
  if (controllerRef !== "refs/heads/main" || controllerSha !== mainSha) throw new Error("QA candidate failed: controller")
  if (actor !== TIER_3_AUTHORIZER) throw new Error("QA candidate failed: ivan-authorization")
  if (runAttempt !== "1") throw new Error("QA candidate failed: fresh-dispatch-required")
