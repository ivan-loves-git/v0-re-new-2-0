...
const VALIDATION_REF = "ncsnoinizxwdcbarxzzl"
const VALIDATION_PROJECT = "renew-overnight-validation-20260820"
const VALIDATION_HOST = "renew-overnight-validation-git-e43dfc-myworkmail4-pngs-projects.vercel.app"
...
export function buildFixtureManifest(runId) {
  ...
  return {
    ...
    fixturePrefix,
    actors: {
      staff: { userId: staffUserId, email: `${fixturePrefix}-staff@test.invalid` },
      portal: { userId: portalUserId, email: `${fixturePrefix}-portal@test.invalid`, repreneurId: ids.portalRepreneur },
      applicant: { email: `delivered+${fixturePrefix.toLowerCase()}@resend.dev` },
      staffCreated: { email: `${fixturePrefix}-staff-created@test.invalid` },
    },
    ids,
    referenceCode,
    databaseRows: [
      ...
      row("wave_journey_settings", "true", fixturePrefix),
    ],
...
function validOrigin(value) {
  try {
    ...
    return url.protocol === "https:" && url.origin === value && url.hostname === VALIDATION_HOST
  } catch {
    return false
  }
}
...
export function assertSafeQaRuntime(env) {
  ...
  if (!validRef(projectRef) || projectRef !== VALIDATION_REF) throw new Error("QA runtime isolation failed: project-ref")
  let apiRef = ""
  try {
...
export function validateLiveEvidence({ expectedRef, expectedOrigin, expectedSha, evidence }) {
  ...
  if ([supabase.customerRows, supabase.betterAuthUsers, supabase.supabaseAuthUsers, supabase.storageObjects].some((count) => count !== 0)) fail("non-empty")
