...
describe("opportunity closure history", () => {
  it("accepts only the required canonical closure reasons", () => {
    expect(OPPORTUNITY_CLOSURE_REASON_OPTIONS.map((option) => option.value)).toEqual([
      "stale",
      "sold",
      "signed_repreneur",
      "paused_cabinet",
  ...
  it("requires a valid reason and prevents the generic edit action from closing records", () => {
    const closeAction = functionSource("lib/actions/opportunities.ts", "closeOpportunity")
    const updateAction = functionSource("lib/actions/opportunities.ts", "updateOpportunity")
    const controls = source("components/opportunities/opportunity-closure-controls.tsx")

    expect(closeAction).toContain("isOpportunityClosureReason(reason)")
    expect(closeAction).toContain("close_opportunity_with_reason")
    expect(updateAction).toContain('currentStatus !== "closed" && requestedStatus === "closed"')
    expect(controls).toContain("disabled={!selectedReason || isSubmitting}")
  })

  it("keeps closure rows immutable and reopens without touching their history", () => {
    const migration = source("scripts/069_opportunity_closure_history.sql")
    const reopenAction = functionSource("lib/actions/opportunities.ts", "reopenOpportunity")

    expect(migration).toContain("CREATE TABLE IF NOT EXISTS public.opportunity_closure_history")
    expect(migration).toContain("ON DELETE RESTRICT")
    expect(migration).toContain("BEFORE UPDATE OR DELETE ON public.opportunity_closure_history")
    expect(migration).toContain("opportunity_closure_history_is_immutable")
    expect(reopenAction).toContain('status: "active"')
    expect(reopenAction).toContain('.eq("status", "closed")')
    expect(reopenAction).not.toContain("opportunity_closure_history")
  })

  it("keeps closure history actions behind staff authorization", () => {
    for (const functionName of ["getOpportunityClosureHistory", "closeOpportunity", "reopenOpportunity"]) {
      const action = functionSource("lib/actions/opportunities.ts", functionName)
      expect(action.indexOf("requireStaffAccess")).toBeGreaterThanOrEqual(0)
      expect(action.indexOf("requireStaffAccess")).toBeLessThan(action.indexOf("createAdminClient"))
    }
  })
})
