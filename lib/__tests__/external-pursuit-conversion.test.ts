...
describe("W-109 External Pursuit conversion", () => {
  ...
  it("implements immutable one-way conversion and rejects unsafe lifecycle/source paths", () => {
    ...
    expect(migration.match(/actor_role IS DISTINCT FROM 'staff'/g)).toHaveLength(3)
    expect(migration).not.toMatch(/actor_role\s*<>\s*'staff'/)
    expect(rehearsal).toContain("w109_unknown_conversion_actor_was_allowed")
    expect(rehearsal).toContain("w109_missing_preflight_actor_was_allowed")
    expect(rehearsal).toContain("w109_unknown_fulfillment_actor_was_allowed")
