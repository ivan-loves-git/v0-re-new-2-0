...
describe("W-147 Supabase browser-role boundary", () => {
  it("revokes browser access by default and permits only explicit read exceptions", () => {
    ...
    expect(migration).toContain("ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE ALL ON TABLES FROM anon, authenticated;")
    ...
    expect(migration).not.toMatch(/GRANT\s+(?:ALL|INSERT|UPDATE|DELETE)[\s\S]*TO anon, authenticated/i)
  })
