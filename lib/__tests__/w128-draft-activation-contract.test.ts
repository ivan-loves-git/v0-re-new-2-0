...
const rehearsal = readFileSync(
  path.join(process.cwd(), "scripts/rehearse-w128-draft-opportunity-activation.sh"),
  "utf8"
)
...
describe("W-128 Draft activation contract", () => {
  ...
  it("rehearses the full activation, drift, rollback, and audit path in Verify", () => {
    ...
    expect(verifyWorkflow).toContain("postgres:17-bookworm")
    expect(verifyWorkflow).toContain("PG_BIN=/usr/lib/postgresql/17/bin bash scripts/rehearse-w128-draft-opportunity-activation.sh")
    expect(rehearsal).toContain('tmp_root="${TMPDIR:-/tmp}"')
    expect(rehearsal).toContain('database_superuser="renew_rehearsal_admin"')
    expect(rehearsal).not.toContain("mktemp -d /private/tmp/")
    expect(rehearsal).toContain('-k $cluster_dir')
    expect(rehearsal).toContain('sed -n \'1,120p\' "$cluster_dir/postgres.log" >&2')
  })
})
