...
describe("External Pursuit attachment actions", () => {
  ...
  it("reports ambiguous registration cleanup instead of claiming upload success", async () => {
    mocks.rpc.mockResolvedValueOnce({ data: null, error: null }).mockResolvedValueOnce({ data: null, error: new Error("response lost") })
    mocks.remove.mockResolvedValue({ data: null, error: new Error("storage unavailable") })
    await expect(uploadExternalPursuitAttachment("dossier-1", uploadForm(), key(3))).resolves.toMatchObject({ success: false, message: expect.stringMatching(/cleanup needs staff attention/i) })
  })
  ...
  it("stops individual deletion before metadata finalization when object removal fails", async () => {
    ...
    mocks.remove.mockResolvedValue({ data: null, error: new Error("storage unavailable") })
    await expect(deleteExternalPursuitAttachment("dossier-1", "attachment-1", key(4))).resolves.toMatchObject({ success: false })
  ...
  it("stops dossier fulfillment before metadata/tombstone when any object cleanup fails", async () => {
    ...
    mocks.remove.mockResolvedValue({ data: null, error: new Error("storage unavailable") })
    await expect(fulfillExternalPursuitDeletionWithAttachments("dossier-1", key(5))).resolves.toMatchObject({ success: false })
  ...
  it("routes the legacy fulfillment action through attachment cleanup before tombstoning", async () => {
    ...
    await expect(fulfillExternalPursuitDeletion("dossier-1", key(6))).resolves.toMatchObject({ success: true, pursuitId: "dossier-1" })
