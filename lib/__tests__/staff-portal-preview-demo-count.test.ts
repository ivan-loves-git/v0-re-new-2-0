...
describe("Staff Portal Preview DEMO counts", () => {
  beforeEach(() => {
    ...
    mocks.from.mockImplementation((table: string) => {
      ...
      if (table === "opportunity_matches") return query({
        data: [
          { repreneur_id: "repreneur-1", opportunity: { is_demo: false } },
          { repreneur_id: "repreneur-1", opportunity: { is_demo: true } },
        ],
        error: null,
      })
  ...
  it("counts only explicitly non-DEMO opportunities even if a query adapter returns both", async () => {
