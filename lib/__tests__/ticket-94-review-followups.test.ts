...
describe("Ticket #94 review follow-ups", () => {
  ...
  it("keeps a rollback-compatible conversion while rehearsing the strict candidate path", () => {
    ...
    expect(rehearsal).toContain("w164_direct_v2_real_demo_creation_failed")
    expect(rehearsal).toContain("w164_edit_accepted_demo_classification")
  ...
  it("has a separately rehearsed post-deploy cutover and ordered rollback", () => {
    ...
    expect(source("docs/data-models/ma-advisory-data-model-v1.md")).toContain("strict eight-argument `convert_external_pursuit_to_opportunity` path")
  })
})
