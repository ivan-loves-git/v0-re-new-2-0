...
describe("ExternalPursuitBoard component", () => {
  ...
  it("captures completed actions only after confirmed success, never before retry recovery", () => {
    ...
    expect(attachments.indexOf("await uploadPrivateDocument")).toBeLessThan(attachments.indexOf("captureExternalPursuitCompleted(role, \"upload\")"))
    expect(attachments).toContain("} catch (error) {")
