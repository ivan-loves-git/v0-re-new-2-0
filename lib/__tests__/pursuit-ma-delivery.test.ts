...
const review = { recipientEmail: "contact@re-new.invalid", templateVersion: "template-v1", actorId: "staff-id" }
...
function query(result: unknown) {
  const q = { select: () => q, eq: () => q, single: () => Promise.resolve({ data: result, error: null }), maybeSingle: () => Promise.resolve({ data: result, error: null }), order: () => Promise.resolve({ data: result, error: null }) }
  return q
}
beforeEach(() => {
  vi.clearAllMocks(); m.order.length = 0
  m.staff.mockResolvedValue({ user: { id: "staff-id", email: "staff@re-new.invalid" } })
  m.from.mockImplementation((table) => query(table === "email_templates" ? { template_key: "ma_nda_info_memo_request", subject: "subject", body_markdown: "body", body_editable: true, is_active: true } : table === "opportunities" ? opportunity : [{ id: "match", status: "active_pursuit", repreneur: { is_demo: false, first_name: "Buyer" } }]))
...
describe("E4/E7 canonical M&A delivery", () => {
  ...
  it("does not call the provider while the handoff lease is in flight", async () => {
    ...
    expect((await sendMaSourceWorkflowEmailPayload("opp", payload, handoff, review)).success).toBe(false)
  ...
  it("retains the same operation after ambiguous transport and does not finalize failed", async () => {
    ...
    expect(await sendMaSourceWorkflowEmailPayload("opp", payload, handoff, review)).toMatchObject({ success: false, operationState: "pending" })
