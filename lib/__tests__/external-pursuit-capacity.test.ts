...
describe("W-110 External Pursuit capacity", () => {
  ...
  it("enforces Paris boundaries, full distributions, role boundaries and the excluded linked bucket in SQL", () => {
    ...
    expect(migration).toContain("actor_role IS DISTINCT FROM 'staff'")
    ...
    expect(rehearsal).toContain("w110_unknown_capacity_actor_was_allowed")
    expect(rehearsal).toContain("w110_missing_capacity_actor_was_allowed")
