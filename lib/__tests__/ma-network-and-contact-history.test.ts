...
describe("M&A networks and contact history", () => {
  it("keeps networks canonical, optional, and staff-only", () => {
    ...
    expect(migration).toContain(
      "REVOKE ALL ON TABLE public.ma_source_networks",
    );
  });

  it("snapshots opportunity attribution before contacts can move", () => {
    const migration = source(
      "scripts/075_preserve_ma_contact_move_history.sql",
    );
  ...
  it("exposes separate staff firm and contact routes while keeping the old route compatible", () => {
    ...
    expect(sidebar).toContain('name: "Firms", href: "/opportunities/ma/firms"');
    expect(sidebar).toContain(
      'name: "Contacts", href: "/opportunities/ma/contacts"',
    );
    expect(legacyRoute).toContain('redirect("/opportunities/ma/firms")');
    expect(firmsPage).toContain('title="M&A firms"');
    expect(contactsPage).toContain('title="M&A contacts"');
  });
});
