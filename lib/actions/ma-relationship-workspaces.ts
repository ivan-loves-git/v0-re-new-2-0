...
const STALE_CANDIDATE_STATUSES = new Set(["draft", "active", "paused"]);
const OPEN_OPPORTUNITY_STATUSES = new Set(["active", "paused"]);
...
function isCandidateStale(
  opportunity: { id: string; status: string; date_added: string | null },
  activePursuitIds: Set<string>,
  now: Date,
) {
  if (!STALE_CANDIDATE_STATUSES.has(opportunity.status)) return false;
  if (!opportunity.date_added) return false;
  const date = new Date(opportunity.date_added);
  ...
  return (
    daysOpen >= STALE_OPPORTUNITY_DAYS && !activePursuitIds.has(opportunity.id)
  );
}
...
function buildIndicators(
  ...
  return {
    ...
    closedOpportunities: opportunities.filter(
...
async function getWorkspaceRows(officeIds: string[]) {
  ...
  for (const row of opportunitiesResult.data ?? []) {
    ...
    entries.push({
      ...
      isCandidateStale: isCandidateStale(row, activePursuitIds, now),
    });
...
export async function getMaFirmWorkspace(
  ...
  for (const contacts of contactsByOffice.values())
    for (const contact of contacts) {
      const current = contactsById.get(contact.id);
      if (!current || contact.isActive) contactsById.set(contact.id, contact);
    }
...
export async function updateMaRelationshipWorkspaceNotes(
  ...
  return { success: true, message: "Internal notes saved with staff audit." };
}
