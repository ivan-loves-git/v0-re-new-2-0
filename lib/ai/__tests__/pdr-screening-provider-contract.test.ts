...
import { generatePdrScreening, PDR_SCREENING_MAX_OUTPUT_TOKENS, PDR_SCREENING_REASONING_EFFORT } from "@/lib/ai/pdr-screening"
...
describe("PDR screening provider contract", () => {
  it("sends only compact allowlisted request and governance fields with an opaque safety id", async () => {
    ...
    expect(PDR_SCREENING_REASONING_EFFORT).toBe("low")
