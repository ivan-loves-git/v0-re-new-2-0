...
Keep affectedUsers, desiredOutcome, successSignal, problemFraming, and technicalImpact to one concise sentence each. Keep every clarification question, constraint/non-goal, success criterion, and unknown as one short list item. If classification is bug, ask zero, one, or two short questions only when a missing fact would materially improve reproduction, impact, or routing; a complete obvious bug has zero questions. Ask only about observed versus expected behaviour, affected page or workflow step, affected role or group, recurrence or timing, or a safe redacted screenshot or error reference. Do not use a fixed checklist. Never ask for credentials, passwords, codes, tokens, raw client or customer data, confidential documents, or personal/contact information. For every other classification, ask one to five questions. If uncertain, say so in unknowns and lower confidence. Return only the requested structured output.`
}
