...
AS $$
  SELECT ENCODE(extensions.digest(CONVERT_TO(JSONB_BUILD_OBJECT(
    ...
    'ma_interactions', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.id) FROM public.ma_interactions row_value WHERE row_value.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'ma_interaction_delivery_events', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.id) FROM public.ma_interaction_delivery_events row_value JOIN public.ma_interactions interaction ON interaction.id = row_value.interaction_id WHERE interaction.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'ma_interaction_owner_verification_events', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.id) FROM public.ma_interaction_owner_verification_events row_value JOIN public.ma_interactions interaction ON interaction.id = row_value.interaction_id WHERE interaction.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'ma_contact_email_policy_events', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.id) FROM public.ma_contact_email_policy_events row_value WHERE row_value.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'ma_opportunity_date_correction_events', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.id) FROM public.ma_opportunity_date_correction_events row_value WHERE row_value.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'ma_provisional_source_review_events', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.id) FROM public.ma_provisional_source_review_events row_value WHERE row_value.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'ma_source_email_send_reservations', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.opportunity_id) FROM public.ma_source_email_send_reservations row_value WHERE row_value.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'ma_source_interactions', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.id) FROM public.ma_source_interactions row_value WHERE row_value.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'ma_w039_geography_adoption_evidence', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.run_id, row_value.opportunity_id) FROM public.ma_w039_geography_adoption_evidence row_value WHERE row_value.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'external_pursuit_conversions', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.external_pursuit_id) FROM public.external_pursuit_opportunity_conversions row_value WHERE row_value.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'external_pursuits', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.id) FROM public.external_pursuits row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.id WHERE conversion.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'external_pursuit_contacts', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.id) FROM public.external_pursuit_contacts row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.external_pursuit_id WHERE conversion.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'external_pursuit_notes', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.external_pursuit_id) FROM public.external_pursuit_notes row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.external_pursuit_id WHERE conversion.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'external_pursuit_staff_notes', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.external_pursuit_id) FROM public.external_pursuit_staff_notes row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.external_pursuit_id WHERE conversion.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'external_pursuit_attachments', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.id) FROM public.external_pursuit_attachments row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.external_pursuit_id WHERE conversion.opportunity_id = p_opportunity_id), '[]'::JSONB),
    'external_pursuit_audit_events', COALESCE((SELECT JSONB_AGG(TO_JSONB(row_value) ORDER BY row_value.id) FROM public.external_pursuit_audit_events row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.external_pursuit_id WHERE conversion.opportunity_id = p_opportunity_id), '[]'::JSONB)
...
BEGIN
  ...
  PERFORM 1 FROM public.ma_interactions row_value JOIN w128_rollback_manifest manifest ON manifest.id = row_value.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.ma_interaction_delivery_events row_value JOIN public.ma_interactions interaction ON interaction.id = row_value.interaction_id JOIN w128_rollback_manifest manifest ON manifest.id = interaction.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.ma_interaction_owner_verification_events row_value JOIN public.ma_interactions interaction ON interaction.id = row_value.interaction_id JOIN w128_rollback_manifest manifest ON manifest.id = interaction.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.ma_contact_email_policy_events row_value JOIN w128_rollback_manifest manifest ON manifest.id = row_value.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.ma_opportunity_date_correction_events row_value JOIN w128_rollback_manifest manifest ON manifest.id = row_value.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.ma_provisional_source_review_events row_value JOIN w128_rollback_manifest manifest ON manifest.id = row_value.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.ma_source_email_send_reservations row_value JOIN w128_rollback_manifest manifest ON manifest.id = row_value.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.ma_source_interactions row_value JOIN w128_rollback_manifest manifest ON manifest.id = row_value.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.ma_w039_geography_adoption_evidence row_value JOIN w128_rollback_manifest manifest ON manifest.id = row_value.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.external_pursuit_opportunity_conversions row_value JOIN w128_rollback_manifest manifest ON manifest.id = row_value.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.external_pursuits row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.id JOIN w128_rollback_manifest manifest ON manifest.id = conversion.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.external_pursuit_contacts row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.external_pursuit_id JOIN w128_rollback_manifest manifest ON manifest.id = conversion.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.external_pursuit_notes row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.external_pursuit_id JOIN w128_rollback_manifest manifest ON manifest.id = conversion.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.external_pursuit_staff_notes row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.external_pursuit_id JOIN w128_rollback_manifest manifest ON manifest.id = conversion.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.external_pursuit_attachments row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.external_pursuit_id JOIN w128_rollback_manifest manifest ON manifest.id = conversion.opportunity_id FOR UPDATE OF row_value;
  PERFORM 1 FROM public.external_pursuit_audit_events row_value JOIN public.external_pursuit_opportunity_conversions conversion ON conversion.external_pursuit_id = row_value.external_pursuit_id JOIN w128_rollback_manifest manifest ON manifest.id = conversion.opportunity_id FOR UPDATE OF row_value;
