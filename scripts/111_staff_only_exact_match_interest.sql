-- A staff_only value is a broad discovery compatibility filter, not an
-- override for the exact current portal match. The exact match is allowed to
-- signal interest only while it is one of the same statuses the portal shows.
...
COMMENT ON FUNCTION public.express_opportunity_interest(UUID, UUID, TEXT, TIMESTAMPTZ) IS
  'Atomically creates or reuses one interested opportunity_matches pair for an active broadly visible opportunity or its exact portal-visible staff_only match; it never creates an active pursuit or changes confidentiality.';
