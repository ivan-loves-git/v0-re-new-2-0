...
CREATE TABLE public.historical_pursuit_import_source_bindings (
  ...
  manifest_digest TEXT NOT NULL, source_row_fingerprint TEXT NOT NULL CHECK (source_row_fingerprint ~ '^[0-9a-f]{64}$'), source_payload_digest TEXT NOT NULL CHECK (source_payload_digest ~ '^[0-9a-f]{64}$'), approval_digest TEXT NOT NULL CHECK (approval_digest ~ '^[0-9a-f]{64}$'),
  ...
  IF FOUND THEN
    ...
    IF v_match.status = 'draft' AND v_terminal THEN
      ...
      WHERE id = v_match.id
        ...
        AND pursuit_stage_notes IS NULL AND pursuit_stage_updated_by IS NULL AND pursuit_stage_updated_at IS NULL
        AND nda_notes IS NULL AND nda_updated_by IS NULL AND nda_updated_at IS NULL AND nda_waived_by IS NULL
        AND interest_expressed_at IS NULL AND interest_notification_sent_at IS NULL
