...
CREATE OR REPLACE FUNCTION public.historical_pursuit_import_approval_digest(
  ...
  SELECT encode(extensions.digest(convert_to(
    'fingerprint=' || p_source_row_fingerprint || E'\nrepreneur=' || COALESCE(p_repreneur_id::TEXT, 'null') ||
    E'\nopportunity=' || COALESCE(p_opportunity_id::TEXT, 'null') || E'\nblockers=' ||
    to_json(COALESCE(p_resolution_blockers, '{}'::TEXT[]))::TEXT || E'\nflags=' ||
    to_json(COALESCE(p_review_flags, '{}'::TEXT[]))::TEXT,
    'UTF8'), 'sha256'), 'hex')
$$;
...
BEGIN
  ...
  v_approval_digest := public.historical_pursuit_import_approval_digest(
    p_source_row_fingerprint, p_repreneur_id, p_opportunity_id,
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.historical_pursuit_import_allowlist allowlist WHERE allowlist.source_sha256=p_source_sha256 AND allowlist.source_sheet=p_source_sheet AND allowlist.source_row=p_source_row AND allowlist.manifest_digest=p_manifest_digest AND allowlist.approval_digest=v_approval_digest AND allowlist.source_row_fingerprint=p_source_row_fingerprint) THEN RAISE EXCEPTION 'historical_pursuit_allowlist_mismatch'; END IF;
  ...
  IF FOUND THEN
    ...
    IF v_match.status = 'draft' AND v_terminal THEN
      ...
      WHERE id = v_match.id
        ...
        AND NOT EXISTS (SELECT 1 FROM public.opportunity_pursuit_confidential_grants grant_row WHERE grant_row.match_id = v_match.id AND grant_row.revoked_at IS NULL)
