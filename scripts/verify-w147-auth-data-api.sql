...
BEGIN
  FOR item IN SELECT c.oid, c.relname, pg_get_userbyid(c.relowner) AS owner FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind IN ('r','p') LOOP
    IF item.owner <> 'postgres' THEN RAISE EXCEPTION 'w147_unexpected_public_table_owner:%:%',item.relname,item.owner; END IF;
    ...
    ELSIF has_table_privilege('anon',item.oid,'select,insert,update,delete') OR has_table_privilege('authenticated',item.oid,'select,insert,update,delete') THEN RAISE EXCEPTION 'w147_browser_table_grant:%',item.relname; END IF;
  ...
  END LOOP;
  FOR item IN SELECT c.oid, c.relname, pg_get_userbyid(c.relowner) AS owner FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind='S' LOOP
    IF item.owner <> 'postgres' THEN RAISE EXCEPTION 'w147_unexpected_public_sequence_owner:%:%',item.relname,item.owner; END IF;
  ...
  END LOOP;
  FOR item IN SELECT p.oid, p.proname, pg_get_userbyid(p.proowner) AS owner FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' LOOP
    IF item.owner <> 'postgres' THEN RAISE EXCEPTION 'w147_unexpected_public_function_owner:%:%',item.proname,item.owner; END IF;
