...
-- Existing rows are deliberately untouched. The strict v2 creator is additive:
-- old-app create and seven-argument conversion calls remain deploy/rollback
-- compatible. Candidate code calls only v2, where `is_demo` is mandatory.
BEGIN;
...
-- The additive eight-argument conversion delegates to the strict creator.
-- Keep the seven-argument release endpoint for old-app rollback compatibility.
...
BEGIN
  ...
  saved:=public.create_opportunity_with_office_context_v2('',source_office.id,ARRAY[p_primary_affiliation_id],p_primary_affiliation_id,NULL,'draft',actor,jsonb_build_object('geography_node_id',node.id,'public_title',safe_title,'is_demo',p_is_demo));
