...
async function cleanup() {
  ...
  const { rows } = await client.query<OpeningFixtureCounts>(
    `SELECT
      ...
      (SELECT count(*)::int FROM public."verification" WHERE "identifier" = ANY($7::text[]) OR "value" = ANY($1::text[])) AS verifications,
      ...
      (SELECT count(*)::int FROM public.ma_offices WHERE id = ANY($4::uuid[])) AS offices,
      (SELECT count(*)::int FROM public.ma_contacts WHERE id = ANY($5::uuid[])) AS contacts,
      (SELECT count(*)::int FROM public.opportunity_documents WHERE opportunity_id = ANY($3::uuid[])) AS documents,
      (SELECT count(*)::int FROM storage.objects WHERE name LIKE $6) AS "storageObjects"`,
    [
      syntheticUserIds,
      syntheticRepreneurIds,
      syntheticOpportunityIds,
