...
BEGIN
  ...
  CREATE TEMP TABLE w126_manifest (
    id UUID PRIMARY KEY,
    reference TEXT NOT NULL,
    updated_at TIMESTAMPTZ NOT NULL,
    fingerprint TEXT NOT NULL,
    expected_matches INTEGER NOT NULL,
    expected_active_pursuits INTEGER NOT NULL,
    expected_documents INTEGER NOT NULL,
    expected_artifacts INTEGER NOT NULL
  ) ON COMMIT DROP;

  INSERT INTO w126_manifest VALUES
  ...
  IF (SELECT COUNT(*) FROM w126_manifest) <> v_expected_rows THEN
  ...
  PERFORM 1 FROM public.opportunities o JOIN w126_manifest m ON m.id=o.id FOR UPDATE OF o;
  ...
  SELECT COALESCE(SUM(expected_active_pursuits),0) INTO v_active_rows FROM w126_manifest;
