...
# Disposable PostgreSQL 17 only. Never opens a Supabase project or provider.
repo_root="${RENEW_W174_REPO_ROOT:-}"
if [[ -z "$repo_root" ]]; then
  repo_root="$(git -C "$PWD" rev-parse --show-toplevel 2>/dev/null || true)"
fi
if [[ -z "$repo_root" || ! -d "$repo_root/supabase/schema" ]]; then
  echo "Set RENEW_W174_REPO_ROOT to the platform repository." >&2
  exit 1
fi
pg_bin="${PG_BIN:-/opt/homebrew/opt/postgresql@17/bin}"
port="${RENEW_W174_REHEARSAL_PORT:-55514}"
database_name="renew_w174_rehearsal"
database_superuser="renew_w174_rehearsal_admin"
temp_base="${TMPDIR:-/tmp}"
if [[ "$temp_base" != /* || ! -d "$temp_base" ]]; then
  echo "TMPDIR must be an existing absolute directory." >&2
  exit 1
fi
cluster_dir="$(mktemp -d "$temp_base/renew-w174-rehearsal.XXXXXX")"

cleanup() {
  if [[ ! -d "$cluster_dir" || "$cluster_dir" != "$temp_base"/renew-w174-rehearsal.* ]]; then
  ...
  if [[ "${RENEW_W174_RETAIN_CLUSTER:-0}" == "1" ]]; then
...
"${psql[@]}" --file "$repo_root/scripts/rehearsals/w174-memo-feedback-before.sql" >/dev/null
"${psql[@]}" --file "$repo_root/supabase/migrations/20260922125444_w174_memo_feedback_evidence_type.sql" >/dev/null
"${psql[@]}" --file "$repo_root/supabase/migrations/20260922125456_w174_memo_feedback_reminder.sql" >/dev/null
"${psql[@]}" --file "$repo_root/scripts/rehearsals/w174-memo-feedback-after.sql"
"${psql[@]}" --file "$repo_root/scripts/rehearsals/w174-memo-feedback-delivery-after.sql"
"${psql[@]}" --file "$repo_root/scripts/rehearsals/w174-memo-feedback-cancellation-after.sql"
echo "W174 exact-grant memo feedback local PG17 rehearsal passed"
