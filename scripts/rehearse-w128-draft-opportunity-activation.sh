...
tmp_root="${TMPDIR:-/tmp}"
cluster_dir="$(mktemp -d "${tmp_root%/}/renew-w128-draft-activation.XXXXXX")"
port="${W128_DRAFT_ACTIVATION_REHEARSAL_PORT:-55447}"
database_superuser="renew_rehearsal_admin"
cleanup() {
...
for binary in initdb pg_ctl postgres createdb psql; do
...
if ! "$pg_bin/pg_ctl" -D "$cluster_dir" -l "$cluster_dir/postgres.log" -o "-p $port -h 127.0.0.1 -k $cluster_dir" -w start >/dev/null; then
  sed -n '1,120p' "$cluster_dir/postgres.log" >&2
  exit 1
fi
"$pg_bin/createdb" -h 127.0.0.1 -p "$port" -U "$database_superuser" w128_rehearsal
psql=("$pg_bin/psql" -v ON_ERROR_STOP=1 -h 127.0.0.1 -p "$port" -U "$database_superuser" -d w128_rehearsal)
